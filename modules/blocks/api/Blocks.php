...
/**
 * Blocks module API
 * @package yii\easyii\modules\blocks\api
 *
 * @method static CategoryObject cat(mixed $id_slug) Get blocks category by id or slug
 * @method static array tree() Get blocks categories as tree
 * @method static array cats() Get blocks categories as flat array
 * @method static array items(array $options = []) Get list of blockss as BlocksObject objects
 * @method static BlocksObject get(mixed $id_slug) Get blocks object by id or slug
 * @method static mixed last(int $limit = 1) Get last blockss
...
class Blocks extends \yii\easyii\components\API
{
    private $_cats;
    private $_items;
    private $_adp;
    ...
    public function api_items($options = [])
    {
        if(!$this->_items){
            ...
            foreach($this->_adp->models as $model){
                $this->_items[] = new BlockObject($model);
            }
        }
        return $this->_items;
    }
    ...
    {
        $blocks = Item::find()->where(['or', 'item_id=:id_slug', 'slug=:id_slug'], [':id_slug' => $id_slug])->status(Item::STATUS_ON)->one();
        if($blocks) {
            $blocks->updateCounters(['views' => 1]);
            return new BlocksObject($blocks);
        } else {
            return null;
        }
    }
